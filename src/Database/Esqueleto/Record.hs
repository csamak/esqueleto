...
makeSqlMaybeToMaybeInstance RecordInfo {..} = do
  ...
  let toMaybeIdentityDec = FunD 'toMaybe [ Clause [] (NormalB (VarE 'id)) []]
      overlap = Nothing
      instanceConstraints = []
      instanceType = (ConT ''ToMaybe) `AppT` (ConT sqlMaybeName)
  pure $ InstanceD overlap instanceConstraints instanceType [sqlMaybeToMaybeTDec', toMaybeIdentityDec]
                  ...
                  DoE
                    ...
                    (statements ++ [
                      NoBindS $ AppE (VarE 'pure) (
                        CondE
                          (AppE (VarE 'or) (ListE $ fmap (\(n, _) -> AppE (VarE 'isJust) (VarE n)) fieldExps))
